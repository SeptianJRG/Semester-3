...
_0x0:
	.DB  0x25,0x64,0x0,0x44,0x72,0x6A,0x64,0x3A
	.DB  0x20,0x0,0x25,0x2E,0x31,0x66,0x20,0x43
	.DB  0x0,0xA,0x4E,0x6F,0x72,0x6D,0x61,0x6C
	.DB  0x0,0xA,0x42,0x61,0x68,0x61,0x79,0x61
	.DB  0x20,0x73,0x65,0x64,0x61,0x6E,0x67,0x0
	.DB  0xA,0x4B,0x72,0x69,0x74,0x69,0x73,0x0
_0x2000000:
...
; .FEND
;
;unsigned char data_suhu, xstring[5];
;float derajat_suhu;
;
;
;void main(void)
...
; 0000 0097 // ADC Clock frequency: 750,000 kHz
; 0000 0098 // ADC Voltage Reference: AREF pin
; 0000 0099 // ADC Auto Trigger Source: ADC Stopped
; 0000 009A // Only the 8 most significant bits of
; 0000 009B // the AD conversion result are used
...
; 0000 00B8       lcd_clear();
	CALL _lcd_clear
; 0000 00B9         //lcd_putsf("Data suhu: ");
; 0000 00BA         data_suhu = read_adc(0);
	LDI  R26,LOW(0)
	RCALL _read_adc
	MOV  R5,R30
; 0000 00BB         sprintf(xstring,"%d",data_suhu);
	...
	CLR  R23
	CALL SUBOPT_0x0
; 0000 00BC         //lcd_puts(xstring);
; 0000 00BD 
; 0000 00BE         //lcd_gotoxy(0,0);
; 0000 00BF 
; 0000 00C0         lcd_putsf("Drjd: ");
	__POINTW2FN _0x0,3
	CALL _lcd_putsf
; 0000 00C1         derajat_suhu = data_suhu * 5.0 * 100.0 / 3.2 / 255.0;
	MOV  R30,R5
	LDI  R31,0
	CALL SUBOPT_0x1
	__GETD2N 0x40A00000
	...
	__GETD1N 0x437F0000
	CALL __DIVF21
	STS  _derajat_suhu,R30
	STS  _derajat_suhu+1,R31
	STS  _derajat_suhu+2,R22
	STS  _derajat_suhu+3,R23
; 0000 00C2         sprintf(xstring,"%.1f C",derajat_suhu);
	LDI  R30,LOW(_xstring)
	LDI  R31,HIGH(_xstring)
	ST   -Y,R31
	ST   -Y,R30
	...
	ST   -Y,R31
	ST   -Y,R30
	LDS  R30,_derajat_suhu
	LDS  R31,_derajat_suhu+1
	LDS  R22,_derajat_suhu+2
	LDS  R23,_derajat_suhu+3
	CALL SUBOPT_0x0
; 0000 00C3         lcd_puts(xstring);
	LDI  R26,LOW(_xstring)
	LDI  R27,HIGH(_xstring)
	CALL _lcd_puts
; 0000 00C4 
; 0000 00C5         delay_ms(100);
	...
	CALL _delay_ms
; 0000 00C6 
; 0000 00C7         if(derajat_suhu >= 0 && derajat_suhu <= 40){
	LDS  R26,_derajat_suhu+3
	TST  R26
	BRMI _0xA
	CALL SUBOPT_0x2
	BREQ PC+3
...
; 0000 00C8         lcd_putsf("\nNormal");
	...
	RJMP _0x13
; 0000 00C9         }
; 0000 00CA         else if(derajat_suhu > 40 && derajat_suhu <= 50){
_0x9:
	CALL SUBOPT_0x2
	BREQ PC+2
	BRCC PC+2
...
; 0000 00CB         lcd_putsf("\nBahaya sedang");
	...
	RJMP _0x13
; 0000 00CC         }
; 0000 00CD         else if(derajat_suhu > 50){
_0xD:
	CALL SUBOPT_0x3
	BREQ PC+2
	BRCC PC+2
...
SUBOPT_0x2:
	LDS  R26,_derajat_suhu
	LDS  R27,_derajat_suhu+1
	LDS  R24,_derajat_suhu+2
	LDS  R25,_derajat_suhu+3
	__GETD1N 0x42200000
	CALL __CMPF12
	RET
